...
def test_acme_client_new_account(acme_client, requests_mock):
    ...
    new_account_url = f"{url}/newAccount"
    new_nonce_url = f"{url}/newNonce"

    requests_mock.get(url, json={"newAccount": new_account_url, "newNonce": new_nonce_url})
    ...
    with patch("acme_linode_objectstorage.utils.rsa_jwk_public", return_value={"fake": "jwk"}):
...
def test_acme_client_new_order(acme_client, requests_mock):
    ...
    requests_mock.get(
        directory_url,
        json={"newOrder": new_order_url, "newNonce": new_nonce_url, "newAccount": new_account_url},
    )
    requests_mock.head(new_nonce_url, headers={"Replay-Nonce": "test-nonce"})
    requests_mock.post(new_account_url, headers={"Location": "test-location"}, json={})
    requests_mock.post(
        new_order_url,
        status_code=201,
        headers={"Location": "test-order", "Replay-Nonce": "new-nonce"},
        json={},
    )

    with patch("acme_linode_objectstorage.utils.rsa_jwk_public", return_value={"fake": "jwk"}):
        acme_client.new_account(only_existing=True)
...
def test_acme_client_signed_request(acme_client, requests_mock):
    ...
    requests_mock.head(new_nonce_url, headers={"Replay-Nonce": "test-nonce"})
    requests_mock.post(
        new_account_url, headers={"Location": "test-location", "Replay-Nonce": "new-nonce"}, json={}
    )
